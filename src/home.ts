...
<head>
	...
	<style>
		:root { color-scheme:dark; --bg:#14100c; --panel:#1c170f; --raised:#221b11; --ink:#f1e8d6; --muted:#a99a7c; --line:#3c3220; --brass:#c9a04e; --bright:#e8c473; --green:#8fd19e; --red:#f0958f; }
		* { box-sizing:border-box; }
		body { margin:0; min-height:100vh; background:var(--bg); color:var(--ink); font:15px/1.55 system-ui,sans-serif; }
		body::before { content:""; position:fixed; inset:0; pointer-events:none; opacity:.22; background:repeating-linear-gradient(0deg,transparent 0 35px,rgba(201,160,78,.08) 36px),radial-gradient(circle at 75% 15%,rgba(201,160,78,.12),transparent 30%); }
		...
		.kicker,.label,label { color:var(--brass); text-transform:uppercase; letter-spacing:.12em; font:700 11px/1.3 system-ui,sans-serif; }
		...
		.postmark { width:164px; aspect-ratio:1; display:grid; place-content:center; border:3px double var(--brass); border-radius:50%; color:var(--brass); text-align:center; transform:rotate(-6deg); box-shadow:inset 0 0 0 8px var(--bg),inset 0 0 0 9px var(--brass); text-transform:uppercase; letter-spacing:.12em; font-size:10px; }
		.postmark strong { display:block; margin:4px 0; font:700 34px/1 ui-serif,Georgia,serif; letter-spacing:-.03em; }
		.notice { display:flex; gap:12px; margin:22px 0 0; padding:13px 16px; border:1px dashed var(--brass); background:rgba(140,50,48,.12); color:#d8c9ad; }
		...
		.panel { position:relative; background:linear-gradient(135deg,var(--raised),var(--panel)); border:1px solid var(--line); padding:25px; box-shadow:6px 6px 0 #0b0907; }
		.panel::before { content:""; position:absolute; inset:6px; pointer-events:none; border:1px solid rgba(201,160,78,.09); }
		...
		.badge { border:1px solid var(--brass); color:var(--brass); padding:4px 8px; font-size:10px; text-transform:uppercase; letter-spacing:.1em; }
		label { display:block; margin:15px 0 7px; }
		textarea { width:100%; min-height:86px; border:1px solid #57482d; border-radius:0; background:#100d09; color:var(--ink); padding:12px; font:12px/1.55 ui-monospace,"SFMono-Regular",Consolas,monospace; outline:none; resize:vertical; }
		textarea::placeholder { color:#746950; }
		textarea:focus { border-color:var(--bright); box-shadow:0 0 0 2px rgba(201,160,78,.22); }
		#key { min-height:142px; }
		button { width:100%; min-height:48px; margin-top:18px; border:1px solid var(--bright); background:var(--brass); color:#171006; padding:13px 18px; font:800 12px/1 system-ui,sans-serif; letter-spacing:.12em; text-transform:uppercase; cursor:pointer; box-shadow:3px 3px 0 #6d5428; }
		button:hover { background:var(--bright); transform:translate(-1px,-1px); }
		button:focus-visible { outline:2px solid var(--ink); outline-offset:4px; }
		button:disabled { opacity:.55; cursor:wait; transform:none; }
		.result { position:relative; min-height:360px; margin:0; padding:20px; overflow:auto; white-space:pre-wrap; overflow-wrap:anywhere; background:#0e0b08; border:1px solid var(--line); color:var(--muted); font:12px/1.6 ui-monospace,"SFMono-Regular",Consolas,monospace; }
		.result.good { border:2px solid #4a7a58; color:var(--green); }
		.result.bad { border:2px solid #8c3230; color:var(--red); }
		.algorithms { grid-column:1/-1; display:flex; flex-wrap:wrap; gap:8px; align-items:center; padding:18px 24px; }
		.algorithms code { padding:5px 9px; border:1px dashed var(--line); color:#d2c19d; font:11px ui-monospace,monospace; }
		footer { display:flex; justify-content:space-between; gap:20px; margin-top:22px; color:#81755e; font-size:11px; }
